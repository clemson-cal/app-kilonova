...
// ============================================================================
fn _advance_rk_async<H, C>(state: State<C>, hydro: &H, model: &Model, mesh: &Mesh, geometry: &HashMap<BlockIndex, GridGeometry>, dt: f64)
...
where
    ...
    let mut save_primitive_and_scalar = |index, state: &BlockState<C>, geometry: &GridGeometry| {
        ...
        stage_map.insert(index, (p.to_shared(), s.to_shared()));
    };

    for (&index, state) in &state.solution {
        save_primitive_and_scalar(index, state, &geometry[&index]);
    }
    ...
    save_primitive_and_scalar(inner_bnd_index, &inner_bnd_state, &inner_bnd_geom);
    save_primitive_and_scalar(outer_bnd_index, &outer_bnd_state, &outer_bnd_geom);
    ...
    for (&index, state) in &state.solution {
        ...
        let (pl, sl) = &stage_map[&il];
        let (p0, s0) = &stage_map[&i0];
        let (pr, sr) = &stage_map[&ir];
        let pe = concatenate(Axis(0), &[pl.slice(s![-2.., ..]), p0.view(), pr.slice(s![..2, ..])]).unwrap();
        let se = concatenate(Axis(0), &[sl.slice(s![-2.., ..]), s0.view(), sr.slice(s![..2, ..])]).unwrap();
